...
> ✅ **v1.1 확정**: `options`(4개)와 `answers`(단일)는 **`optionId` / `answerId` 오름차순(PK 순)**으로 정렬하여 반환합니다. LLM 파이프라인이 출제 순서대로 INSERT하므로 PK 순 = 출제 순서가 됩니다. 클라이언트는 응답 배열 순서를 그대로 화면에 표시합니다 (객관식 ①②③④ = 배열 인덱스 순서).
