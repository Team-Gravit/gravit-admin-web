...
|---|---|
| `200 OK` | 성공 |
| `400 Bad Request` | 검증 실패 (content 빈 값 등) |
| `404 Not Found` | 문제 없음 |
...
| `problems[].answer` | object | 주관식만 존재. 단일 정답 |

#### 정렬 순서

> ✅ **v1.1 확정**: `options`(4개)와 `answer`(단일)는 **`optionId` / `answerId` 오름차순(PK 순)**으로 정렬하여 반환합니다. LLM 파이프라인이 출제 순서대로 INSERT하므로 PK 순 = 출제 순서가 됩니다. 클라이언트는 응답 배열 순서를 그대로 화면에 표시합니다 (객관식 ①②③④ = 배열 인덱스 순서).
...
> **도메인 제약 (디자인 결정)**: 주관식 정답의 **추가·삭제 API는 제공하지 않음** (Q19 결정). 운영자는 자동 생성된 단일 정답의 content·explanation을 수정만 가능. 정답은 항상 1개이며 백오피스에서 추가·삭제·개수 변경 불가.
...
| 3 | `PATCH /problems/{id}/subjective`의 `answer` 동작 | 단일 정답 객체 부분 업데이트(content/explanation) (7-14) |
...
- **학습 컨텐츠**: 주관식 정답 단일(추가/삭제 없음, 내용만 수정), averageProgress 0~100 확정 (7-14, 7-3)
...
- **⚠️ 와이어프레임 동반 수정 필요**: 주관식 편집 폼(6-7-7 등)의 "정답 추가/삭제 UI" → "단일 정답·내용(콤마 표기)/해설만 수정"