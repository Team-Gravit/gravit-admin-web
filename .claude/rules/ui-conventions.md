...
## Figma 부재 시 디자인 보완 (fill, not invent)
- Figma 프레임에 해당 요소가 없으면, 다음 우선순위로 **자체 완성**한다:
  1. DS-02(해당 화면 시각 명세) → 2. DS-03(상호작용 패턴) → 3. DS-01 토큰 + 기존 구현 화면의 관례.
- 자체 보완한 시각 결정은 리뷰 패킷 §C 에 "Figma 부재 → 명세/관례 기반 보완"으로 **반드시 표시**(사후 검수용).
- 단, 시각만 보완한다. **동작·필드·엔드포인트·검증 규칙은 명세에 없으면 만들지 않는다**(→ [[source-of-truth]] 멈춤 규칙, [[antipatterns]]).
- 명세·DS에도 근거가 전혀 없고 추정이 과한 경우(레이아웃 자체가 불명확 등)는 보완하지 말고 멈춰 질문(🙋🏻).
